...
- **Tool Use / Integrations (RESOLVED)**: Thanks to the newly integrated `adapter.Engine` recursive tool loops and `BeforeTool` hook architecture, Dux *can* natively leverage custom Go tools to check the current directory state or read the file you are working on, making Clippy slightly more context-aware!
